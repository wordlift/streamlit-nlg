...
def main():
    ...
    if button_generate:
        ...
        try:
            results_1 = getResults(user_input, "com", 5, 1,5) # read Google results, make sure here to set the proper Google TLD
            ...
            if page == "WordLift R2-D2 (Pegasus)": # 1) Pegasus
                summary_result = pegasus_summarize(full_body, 200)
            ...
            if page == "WordLift BB-8 (T5)": # 2) T5
                summary_result = summarize(full_body, 200)
                st.subheader('💎 Answer 💎')
